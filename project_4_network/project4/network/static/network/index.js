...
function ViewPost({ post, setIsEditing, setPostId, setShowProfile, setProfileId, setPageNumber, setTitle }) {
	...
	const formatedDateTime = formatDateTime(d);
	...
	return (
		<div className="container-fluid card p-3 m-3 post" id={post.id}>
			...
			<ViewComments post={post} showComment={showComment} comments={comments} setComments={setComments} />
		</div>
	)
}

function CommentInput({ post, comments, setComments }) {

	async function handleCommentInput(event) {
		event.preventDefault();

		const content = event.target.elements.InputContent.value;
		const userId = await getMe();

		console.log("Send comment for post: ", post.id);
		...
		event.target.elements.InputContent.value = "";
	}

	return (
		<div className="containter m-3 comment">
			<form onSubmit={handleCommentInput}>
				<textarea className="form-control" name="InputContent" placeholder="Add comment" autoFocus></textarea>
				<button className="btn btn-primary mt-1" type="submit">Send <i className="fa fa-paper-plane"></i></button>
			</form>
		</div>
	)
}

function ViewComment({ post, comment }) {
	const d = new Date(comment.created_at);
	const formatedDateTime = formatDateTime(d);
	return (
		<div className="comment m-3 p-2">
			<div className="d-flex flex-row">
				<div className="flex-item col-sm-2 mt-3">
					<strong className="text-secondary mt-3">{comment.commenter_name}</strong>
				</div>
				<div className="flex-item col-sm-10">
					<div className="container card">
						<div className="m-2">{comment.content}</div>
						<div className="text-right text-secondary m-2">{formatedDateTime}</div>
					</div>
				</div>
			</div>
		</div>
...
function ViewComments({ post, showComment, comments, setComments }) {
	return showComment ? (
		<div className="comment">
			<CommentInput post={post} comments={comments} setComments={setComments} />
			{comments.map(comment => <ViewComment post={post} comment={comment} key={`comment-${comment.id}`} />)}
		</div>
	) : null;
}

//function ViewComments({ post, showComment, comments }) {
	//return (showComment ? (
		//comments.map(comment => <ViewComment post={post} comment={comment} key={`comment-${comment.id}`} />)
	//) : null)
//}
